...
# ── Service ──────────────────────────────────────────────────────────────
PORT = int(os.environ.get("PORT", 9300))    # PROD
...
# ── Paths (decoupled reads, house pattern) — PROD siblings ───────────────
_DIR = Path(__file__).resolve().parent
ROOT = _DIR.parent.parent                      # repo root
NSX_ALLOC = _DIR.parent / "NS-X_PROD" / "data" / "strategy_alloc.json"
NS5_BLEND = _DIR.parent / "NS-5_PROD" / "data" / "sleeve_blend.json"
NS8_SIGNALS = _DIR.parent / "NS-8_PROD" / "data" / "signals.json"
