...
def run_layer4_options_overlay():
    ...
    # Expand DTE range for ETFs with less frequent expirations
    engine = NSOEOptionEngine(ticker=target_ticker)
    chain = engine.fetch_options_chain(min_dte=20, max_dte=75)
    ...
    targets = engine.screen_premium_targets(target_delta_call=0.30, target_delta_put=-0.30, tolerance=0.10)
