...
## Overview
This document defines the standard deployment procedure for the Alpha Terminal + NS platform. All services share a single monorepo and must be deployed together from a single release branch.
...
├── Project_Sequoia/
│   ├── QA_terminal/          # Alpha Terminal QA
│   └── terminal/             # Alpha Terminal PROD (legacy path)
├── Project_Nine_Street/
│   ├── NS_1_QA/              # NS-1 QA
│   ├── NS-2_QA/              # NS-2 QA
│   ├── NS-2_PROD/            # NS-2 PROD
...
### Automated Deploy Script Template
```bash
#!/bin/bash
...
## 5. Environment Isolation (Launchd & Monitoring)
...
│   └── NS_1_QA/          ← Legacy NS-1 QA
├── project_alpha_env/    ← Shared venv (gitignored)
...
|---|---|---|
| 1.0 | 2026-07-30 | Initial deployment procedure |
| 1.1 | 2026-07-30 | Added NS-2 PROD deployment, Alpha Terminal PROD |

---

## Deployment Procedure Reference

Full production deployment procedure is documented in [DEPLOYMENT.md](DEPLOYMENT.md). Includes:
- Branch strategy (feature vs release branches)
- Step-by-step deployment commands for all 5 services
- Automated deploy script and rollback procedure
- Environment isolation (launchd, monitoring, config)
